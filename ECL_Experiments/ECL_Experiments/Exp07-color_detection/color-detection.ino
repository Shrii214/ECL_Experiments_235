...
eiSensors sensors[] =
{
"accX", &data[0], &poll_acc, &init_IMU, NOT_USED,"accY", &data[1], &poll_acc,
&init_IMU, NOT_USED,
...
if (!sensors[fusion_sensors[i]].status) {ei_printf("%s axis sensor initialization failed.\r\n",
...
int8_t found_axis = 0;
is_fusion = false;
found_axis = ei_find_axis(buff);if(found_axis >= 0) {
...
uint8_t poll_HTS(void) {data[9] = HTS.readTemperature();
